    ...
    public sealed class RijndaelManagedTransform : ICryptoTransform
    {
                                           ...
                                           RijndaelManagedTransformMode transformMode)
        {
            ...
            if (m_cipherMode == CipherMode.CFB) {
                ...
                Buffer.BlockCopy(m_IV, 0, m_shiftRegister, 0, 4*m_Nb);
            }
#endif // FEATURE_CRYPTO
        }
                                       ...
                                       bool fLast)
        {
            ...
            if (fLast)
            {
                ...
                case PaddingMode.None:
                    if (lonelyBytes != 0)
                        throw new CryptographicException(("Cryptography_SSE_InvalidDataSize"));
                    break;
                ...
                fixed (int* encryptKeyExpansion = m_encryptKeyExpansion)
                {
                    fixed (int* T = s_T)
                    {
                        fixed (int* TF = s_TF)
                        {
                            ...
                            Console.WriteLine($"iNumBlocks {iNumBlocks}");
                            ...
                            for (int blockNum = 0; blockNum < iNumBlocks; ++blockNum)
                            {
                                ...
                                if (m_cipherMode == CipherMode.CFB) {
                                    ...
                                    if (blockNum != iNumBlocks - 1 || padSize == 0) {
                                        ...
                                        Buffer.BlockCopy(inputBuffer, workBaseIndex, tmpInputBuffer, 0, lonelyBytes);
                                    ...
                                    Buffer.BlockCopy(outputBuffer, blockNum * m_inputBlockSize, m_shiftRegister, m_blockSizeBytes - m_inputBlockSize, m_inputBlockSize);
                                } else {
#endif // FEATURE_CRYPTO
                                for (int i = 0; i < m_Nb; ++i)
                                {
                                        ...
                                        fixed (int* iTF = s_iTF)
                                        {
                                            ...
                                            for (int blockNum = 0; blockNum < iNumBlocks; ++blockNum)
                                            {
                                                ...
                                                if (m_cipherMode == CipherMode.CFB) {
                                                    ...
                                                    Buffer.BlockCopy(inputBuffer, workBaseIndex, m_shiftRegister, m_blockSizeBytes - m_inputBlockSize, m_inputBlockSize);
                                                } else {
#endif // FEATURE_CRYPTO
