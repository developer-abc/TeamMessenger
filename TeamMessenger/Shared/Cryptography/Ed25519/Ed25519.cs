...
namespace TeamMessenger.Shared.Cryptography
{
     ...
     * https://github.com/hanswolff/ed25519
     * /
    ...
    public class Ed25519
    {
        ...
        public static async Task<byte[]> SignData(byte[] message, byte[] signingKey, byte[] publicKey)
        {
            byte[] h = ComputeHash(signingKey);
            BigInteger a = TwoPowBitLengthMinusTwo;
            for (int i = 3; i < (BitLength - 2); i++)
        ...
        public static bool VerifyData(byte[] signature, byte[] message, byte[] publicKey)
        {
        ...
        public static int KeySize => BitLength;
